    ...
    for instance_key in instance_ids:
        ...
        try:
            ...
            instance_type = ec2.describe_instance_attribute(InstanceId=instance_data["instance_id"], \
                                                    Attribute='instanceType')['InstanceType']['Value']

            # Step 2: Get CloudWatch metrics
            recent_stats = get_metric_stats(instance_id, "CPUUtilization", start_time=now - \
                                            timedelta(hours=1), period=300)
            daily_avg_stats = get_metric_stats(instance_id, "CPUUtilization", start_time=now - \
                                               timedelta(hours=24), period=3600)

            # Step 3: Generate prompt
            prompt = generate_prompt(instance_id, region, uptime, recent_stats, daily_avg_stats, \
                                    instance_type)
            ...
            store_recommendation(sanitized_response,region,start_time,now,uptime,instance_id)
...
def generate_prompt(instance_id, region, uptime, recent_metrics, daily_metrics, instance_type):
    ...
    - Estimate power draw using realistic per-vCPU and per-RAM wattage models:
        - Idle or low CPU vCPU draw ≈ 2-3W per vCPU for older gen (Xen), ≈ 0.8-1.5W for newer (Nitro)
    ...
    "current_instance_type", "estimated_savings_usd_per_day", "percent_cost_savings", "estimated_co2_savings_grams_per_day",
    "percent_co2_savings", "aws_service_type", "suggested_instance_summary"(as list of string in terms
    of size, cpu, cost per day, power consumption per day, with units), "current_instance_summary" (as 
    list of string in terms of size, cpu, cost per day, power consumption per day, with units). Second 
    element has key as "reasoning" and value as all list of reasoning you have produced
...
def store_recommendation(sanitized_response,region,start_time,now,uptime,instance_id):
        ...
        recommendations["synced_on"] = now.isoformat()
